...
output:
    html_document:
        ...
        toc_depth: 3
params:
        configfn: '../test/config/config_SN.yaml'
        location: NULL
---
...
```{r data_setup}
#configfn <- 'test/config_full_large_SN.yaml'
config <- read_yaml(params$configfn)
respath <- dirname(params$configfn)
mydir <- paste0(respath, '/data/', config[['name']])
...
catlink <- function(file, sample, legend) {
    file <- str_replace(file,
                        paste0(respath, '/data/', config['name'], '/'),
                        '')
    cat(paste0('\n\n', respath, '/data/', config['name'], '/\n\n'))
    cat(paste0(
